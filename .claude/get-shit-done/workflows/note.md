...
- **Global scope**: `C:/Mergebine/certacota/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Mergebine/certacota/.claude/notes/`)
...
2. Glob `C:/Mergebine/certacota/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Mergebine/certacota/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Mergebine/certacota/.claude/notes/` — works in any directory
